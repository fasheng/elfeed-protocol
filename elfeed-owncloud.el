...
;; TODO remove
;; (defcustom elfeed-owncloud-url "https://127.0.0.1:443"
;;   "ownCloud server address."
;;   :group 'elfeed-protocols
;;   :type 'string)

;; (defcustom elfeed-owncloud-username ""
;;   "Username for ownCloud server."
;;   :group 'elfeed-protocols
;;   :type 'string)

;; (defcustom elfeed-owncloud-password ""
;;   "Password for ownCloud server."
;;   :group 'elfeed-protocols
;;   :type 'string)

(defcustom elfeed-owncloud-maxsize 2000
...
(defvar elfeed-owncloud-feeds nil
...
(defconst elfeed-owncloud-api-init-unread (concat elfeed-owncloud-api-base "/items?type=3&getRead=false&batchSize=-1"))
(defconst elfeed-owncloud-api-init-starred (concat elfeed-owncloud-api-base "/items?type=2&batchSize=-1"))
(defconst elfeed-owncloud-api-update-all (concat elfeed-owncloud-api-base "/items/updated?type=3&lastModified=%s"))
(defconst elfeed-owncloud-api-update-feed (concat elfeed-owncloud-api-base "/items?type=0&id=%s&getRead=false"))
(defconst elfeed-owncloud-api-update-force-all (concat elfeed-owncloud-api-base "/items?type=3&batchSize=-1"))
...
(defun elfeed-owncloud--init-headers (&optional put-json)
  ...
  (let* ((headers `(("User-Agent" . ,elfeed-user-agent))))
    (push `("Authorization" .
            ,(concat "Basic " (base64-encode-string
                               (concat elfeed-owncloud-username ":"
                                       elfeed-owncloud-password)))) headers)
    (when put-json
...
request. DATA is in string format, if not nil will send PUT request."
  ...
  `(let*  ((use-curl elfeed-use-curl) ; capture current value in closure
           (headers (elfeed-owncloud--init-headers ,data))
           (cb (lambda (status)
...
(defun elfeed-owncloud--parse-feeds ()
  "Parse the result feeds JSON buffer, ensure the point in the right place that
`json-read' could execute. Return `elfeed-owncloud-feeds'."
  (setq elfeed-owncloud-feeds (json-read))
  ;; fill feeds in db
  (let* ((feeds (cdr (assoc 'feeds elfeed-owncloud-feeds))))
...
(defun elfeed-owncloud--update-feed-list ()
  "Update ownCloud News's feed list."
  (elfeed-owncloud-with-fetch
    (concat elfeed-owncloud-url elfeed-owncloud-api-feeds)
    nil (elfeed-owncloud--parse-feeds)))

(defun elfeed-owncloud--get-feed-url (feed-id)
...
(defun elfeed-owncloud--get-feed-id (feed-url)
...
(defun elfeed-owncloud--get-last-modified ()
...
elfeed-db as a mock feed."
  (let* ((urlobj (url-generic-parse-url elfeed-owncloud-url))
         (url (concat "owncloud://" elfeed-owncloud-username  "@" (url-host urlobj)
...
(defun elfeed-owncloud--set-last-modified (last-modified)
...
(defun elfeed-owncloud-is-owncloud-entry (entry)
  "Check if specific ENTRY is fetched from ownCloud News. Return t if
is, or return nil."
  (eq (elfeed-meta entry :source) 'owncloud))

(defun elfeed-owncloud--parse-entries (&optional mark-last-modified)
  ...
  (if elfeed-owncloud-feeds
      (let* ((begin-time (time-to-seconds))
        ...
        (setq entries
              (cl-loop for item across items collect
                       (let* ((id (cdr (assoc 'id item)))
                              ...
                              (feed-url (elfeed-owncloud--get-feed-url feed-id))
        ...
        (when (and mark-last-modified (> max-last-modified 0))
          (elfeed-owncloud--set-last-modified max-last-modified))
...
(defun elfeed-owncloud--do-update (action &optional arg)
...
the ARG is the time-stamp."
  (let* ((url-update-force-all (concat elfeed-owncloud-url
                                       elfeed-owncloud-api-update-force-all))
         (url-init-unread (concat elfeed-owncloud-url
                                  elfeed-owncloud-api-init-unread))
         (url-init-starred (concat elfeed-owncloud-url
    ...
    (cond
     ...
     ((eq action 'update-feed)
      ...
      (setq url (concat elfeed-owncloud-url
     ...
     ((eq action 'timestamp)
      (setq url (concat elfeed-owncloud-url
                        (format elfeed-owncloud-api-update-all arg)))))
    (unless elfeed--inhibit-update-init-hooks
      (run-hooks 'elfeed-update-init-hooks))
    (elfeed-owncloud-with-fetch url nil
      (elfeed-owncloud--parse-entries mark-last-modified)
    ...
    (when (eq action 'init)
      ...
      (elfeed-owncloud-with-fetch url-init-starred nil
        ...
        (elfeed-owncloud--parse-entries)
        (run-hook-with-args 'elfeed-update-hooks url-init-starred)))))

(defun elfeed-owncloud-update-all ()
  (interactive)
  (let* ((last-modified (elfeed-owncloud--get-last-modified)))
    (elfeed-owncloud-with-fetch
     (concat elfeed-owncloud-url elfeed-owncloud-api-feeds) nil
     (elfeed-owncloud--parse-feeds)
     (if last-modified
         (elfeed-owncloud--do-update 'timestamp last-modified)
       (elfeed-owncloud--do-update 'init)))))

;;; TODO
(defun elfeed-owncloud (url callback)
  "ownCloud protocol updater. If first time run, it will initial sync for target
URL, or will only fetch the updated entries since last modified."
  (interactive)
  (let* ((last-modified (elfeed-owncloud--get-last-modified)))
    (elfeed-owncloud-with-fetch
     ;; TODO
...
(defun elfeed-owncloud-update-feed (feed-url)
  "Update entries under special feed in ownCloud News."
  (interactive)
  (let* ((feed-id (elfeed-owncloud--get-feed-id feed-url)))
    (when feed-id
      (elfeed-owncloud--do-update 'update-feed feed-id))))

(defun elfeed-owncloud-update-since (&optional timestamp)
...
1970-01-01 00:00:00 UTC, the default TIMESTAMP point to 24 hours ago."
  ...
  (elfeed-owncloud-with-fetch
   (concat elfeed-owncloud-url elfeed-owncloud-api-feeds) nil
   (elfeed-owncloud--parse-feeds)
   (elfeed-owncloud--do-update 'timestamp timestamp)))

(defun elfeed-owncloud-reinit ()
...
(defun elfeed-owncloud-update-force-all ()
...
states. This may take a long time, ensure `elfeed-curl-timeout' is big enough."
  ...
  (elfeed-owncloud-with-fetch
   (concat elfeed-owncloud-url elfeed-owncloud-api-feeds) nil
   (elfeed-owncloud--parse-feeds)
   (elfeed-owncloud--do-update 'update-force-all)))

(defun elfeed-owncloud-mark-read (entry)
  "Notify special ownCloud News entry as read."
  (let* ((id (elfeed-meta entry :id))
         (url (concat elfeed-owncloud-url
                      (format elfeed-owncloud-api-read id))))
    (elfeed-owncloud-with-fetch url "{}")))

(defun elfeed-owncloud-mark-unread (entry)
  "Notify special ownCloud News entry as unread."
  (let* ((id (elfeed-meta entry :id))
         (url (concat elfeed-owncloud-url
                      (format elfeed-owncloud-api-unread id))))
    (elfeed-owncloud-with-fetch url "{}")))

(defun elfeed-owncloud-mark-star (entry)
  "Notify special ownCloud News entry as starred."
  (let* ((feed-id (elfeed-meta entry :feed-id))
         (guid-hash (elfeed-meta entry :guid-hash))
         (url (concat elfeed-owncloud-url (format elfeed-owncloud-api-star
                                                feed-id guid-hash))))
    (elfeed-owncloud-with-fetch url "{}")))

(defun elfeed-owncloud-mark-unstar (entry)
  "Notify special ownCloud News entry as unstarred."
  (let* ((feed-id (elfeed-meta entry :feed-id))
         (guid-hash (elfeed-meta entry :guid-hash))
         (url (concat elfeed-owncloud-url (format elfeed-owncloud-api-unstar
                                                feed-id guid-hash))))
    (elfeed-owncloud-with-fetch url "{}")))

(defun elfeed-owncloud-mark-read-multi (entries)
  "Notify multiple ownCloud news entries to be read."
  (let* ((url (concat elfeed-owncloud-url elfeed-owncloud-api-read-multi))
         (ids (cl-loop for entry in entries collect
                       (when (elfeed-owncloud-is-owncloud-entry entry)
                         (elfeed-meta entry :id))))
         (data (json-encode-list (list (cons 'items ids)))))
    (when ids
      (elfeed-owncloud-with-fetch url data))))

(defun elfeed-owncloud-mark-unread-multi (entries)
  "Notify multiple ownCloud news entries to be unread."
  (let* ((url (concat elfeed-owncloud-url elfeed-owncloud-api-unread-multi))
         (ids (cl-loop for entry in entries collect
                       (when (elfeed-owncloud-is-owncloud-entry entry)
                         (elfeed-meta entry :id))))
         (data (json-encode-list (list (cons 'items ids)))))
...
(defun elfeed-owncloud-mark-star-multi (entries)
  "Notify multiple ownCloud news entries to be starred."
  (let* ((url (concat elfeed-owncloud-url elfeed-owncloud-api-star-multi))
...
(defun elfeed-owncloud-mark-unstar-multi (entries)
  "Notify multiple ownCloud news entries to be unstarred."
  (let* ((url (concat elfeed-owncloud-url elfeed-owncloud-api-unstar-multi))
         (items (cl-loop for entry in entries collect
                         (when (elfeed-owncloud-is-owncloud-entry entry)
                           (let* ((feed-id (elfeed-meta entry :feed-id))
                                  (guid-hash (elfeed-meta entry :guid-hash)))
...
(defun elfeed-owncloud-sync-tag-multi (entries tag action)
...
`elfeed-owncloud-star-tag', ACTION could be add or remove."
  (cond
   ((eq action 'add)
    (cond
     ((eq tag 'unread) (elfeed-owncloud-mark-unread-multi entries))
     ((eq tag elfeed-owncloud-star-tag) (elfeed-owncloud-mark-star-multi entries))))
   ((eq action 'remove)
    (cond
     ((eq tag 'unread) (elfeed-owncloud-mark-read-multi entries))
     ((eq tag elfeed-owncloud-star-tag) (elfeed-owncloud-mark-unstar-multi entries))))))

(defun elfeed-owncloud-pre-tag (entries &rest tags)
  ...
  (dolist (tag tags)
    (let* ((entries-modified (cl-loop for entry in entries
                                      ...
                                      collect entry)))
      (elfeed-owncloud-sync-tag-multi entries-modified tag 'add))))

(defun elfeed-owncloud-pre-untag (entries &rest tags)
  "Sync unread/starred stats before tags removed."
  (dolist (tag tags)
    (let* ((entries-modified (cl-loop for entry in entries
                                      when (elfeed-tagged-p tag entry)
                                      collect entry)))
      (elfeed-owncloud-sync-tag-multi entries-modified tag 'remove))))
