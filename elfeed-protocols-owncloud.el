;;; elfeed-protocol-owncloud.el --- owncloud news api for elfeed -*- lexical-binding: t; -*-
...
(defcustom elfeed-protocol-owncloud-url "https://127.0.0.1:443"
  "ownCloud server address."
  :group 'elfeed
  :type 'string)

(defcustom elfeed-protocol-owncloud-username ""
  "Username for ownCloud server."
  :group 'elfeed
  :type 'string)

(defcustom elfeed-protocol-owncloud-password ""
...
(defvar elfeed-protocol-owncloud-feeds nil
...
(defconst elfeed-protocol-owncloud-api-init-unread (concat elfeed-protocol-owncloud-api-base "/items?type=3&getRead=false&batchSize=-1"))
(defconst elfeed-protocol-owncloud-api-init-starred (concat elfeed-protocol-owncloud-api-base "/items?type=2&batchSize=-1"))
(defconst elfeed-protocol-owncloud-api-update-all (concat elfeed-protocol-owncloud-api-base "/items/updated?type=3&lastModified=%s"))
(defconst elfeed-protocol-owncloud-api-update-feed (concat elfeed-protocol-owncloud-api-base "/items?type=0&id=%s&getRead=false"))
(defconst elfeed-protocol-owncloud-api-update-force-all (concat elfeed-protocol-owncloud-api-base "/items?type=3&batchSize=-1"))
...
(defun elfeed-protocol-owncloud--init-headers (&optional put-json)
  ...
  (let* ((headers `(("User-Agent" . ,elfeed-user-agent))))
    (push `("Authorization" .
            ,(concat "Basic " (base64-encode-string
                               (concat elfeed-protocol-owncloud-username ":"
                                       elfeed-protocol-owncloud-password)))) headers)
    (when put-json
...
request. DATA is in string format, if not nil will send PUT request."
  ...
  `(let*  ((use-curl elfeed-use-curl) ; capture current value in closure
           (headers (elfeed-protocol-owncloud--init-headers ,data))
           (cb (lambda (status)
...
(defun elfeed-protocol-owncloud--parse-feeds ()
  "Parse the result feeds JSON buffer, ensure the point in the right place that
`json-read' could execute. Return `elfeed-protocol-owncloud-feeds'."
  (setq elfeed-protocol-owncloud-feeds (json-read))
  ;; fill feeds in db
  (let* ((feeds (cdr (assoc 'feeds elfeed-protocol-owncloud-feeds))))
...
(defun elfeed-protocol-owncloud--update-feed-list ()
  "Update ownCloud News's feed list."
  (elfeed-protocol-owncloud-with-fetch
    (concat elfeed-protocol-owncloud-url elfeed-protocol-owncloud-api-feeds)
    nil (elfeed-protocol-owncloud--parse-feeds)))

(defun elfeed-protocol-owncloud--get-feed-url (feed-id)
...
(defun elfeed-protocol-owncloud--get-feed-id (feed-url)
...
(defun elfeed-protocol-owncloud--get-last-modified ()
...
elfeed-db as a mock feed."
  (let* ((urlobj (url-generic-parse-url elfeed-protocol-owncloud-url))
         (url (concat "owncloud://" elfeed-protocol-owncloud-username  "@" (url-host urlobj)
...
(defun elfeed-protocol-owncloud--set-last-modified (last-modified)
...
(defun elfeed-protocol-owncloud-is-owncloud-entry (entry)
  "Check if specific ENTRY is fetched from ownCloud News. Return t if
is, or return nil."
  (eq (elfeed-meta entry :source) 'owncloud))

(defun elfeed-protocol-owncloud--parse-entries (&optional mark-last-modified)
  ...
  (if elfeed-protocol-owncloud-feeds
      (let* ((begin-time (time-to-seconds))
        ...
        (setq entries
              (cl-loop for item across items collect
                       (let* ((id (cdr (assoc 'id item)))
                              ...
                              (feed-url (elfeed-protocol-owncloud--get-feed-url feed-id))
        ...
        (when (and mark-last-modified (> max-last-modified 0))
          (elfeed-protocol-owncloud--set-last-modified max-last-modified))
...
(defun elfeed-protocol-owncloud--do-update (action &optional arg)
...
the ARG is the time-stamp."
  (let* ((url-update-force-all (concat elfeed-protocol-owncloud-url
                                       elfeed-protocol-owncloud-api-update-force-all))
         (url-init-unread (concat elfeed-protocol-owncloud-url
                                  elfeed-protocol-owncloud-api-init-unread))
         (url-init-starred (concat elfeed-protocol-owncloud-url
    ...
    (cond
     ...
     ((eq action 'update-feed)
      ...
      (setq url (concat elfeed-protocol-owncloud-url
     ...
     ((eq action 'timestamp)
      (setq url (concat elfeed-protocol-owncloud-url
                        (format elfeed-protocol-owncloud-api-update-all arg)))))
    (unless elfeed--inhibit-update-init-hooks
      (run-hooks 'elfeed-update-init-hooks))
    (elfeed-protocol-owncloud-with-fetch url nil
      (elfeed-protocol-owncloud--parse-entries mark-last-modified)
    ...
    (when (eq action 'init)
      ...
      (elfeed-protocol-owncloud-with-fetch url-init-starred nil
        ...
        (elfeed-protocol-owncloud--parse-entries)
        (run-hook-with-args 'elfeed-update-hooks url-init-starred)))))

(defun elfeed-protocol-owncloud-update-all ()
  "Update entries in ownCloud News, if first time run, will initial sync, or
will fetch the updated entries."
  (interactive)
  (let* ((last-modified (elfeed-protocol-owncloud--get-last-modified)))
    (elfeed-protocol-owncloud-with-fetch
     (concat elfeed-protocol-owncloud-url elfeed-protocol-owncloud-api-feeds) nil
     (elfeed-protocol-owncloud--parse-feeds)
     (if last-modified
         (elfeed-protocol-owncloud--do-update 'timestamp last-modified)
       (elfeed-protocol-owncloud--do-update 'init)))))

;;; TODO
(defun elfeed-protocol-owncloud (url callback)
  "TODO"
  (interactive)
  )

(defun elfeed-protocol-owncloud-update-feed (feed-url)
  "Update entries under special feed in ownCloud News."
  (interactive)
  (let* ((feed-id (elfeed-protocol-owncloud--get-feed-id feed-url)))
    (when feed-id
      (elfeed-protocol-owncloud--do-update 'update-feed feed-id))))

(defun elfeed-protocol-owncloud-update-since (&optional timestamp)
...
1970-01-01 00:00:00 UTC, the default TIMESTAMP point to 24 hours ago."
  ...
  (elfeed-protocol-owncloud-with-fetch
   (concat elfeed-protocol-owncloud-url elfeed-protocol-owncloud-api-feeds) nil
   (elfeed-protocol-owncloud--parse-feeds)
   (elfeed-protocol-owncloud--do-update 'timestamp timestamp)))

(defun elfeed-protocol-owncloud-reinit ()
...
(defun elfeed-protocol-owncloud-update-force-all ()
...
states. This may take a long time, ensure `elfeed-curl-timeout' is big enough."
  ...
  (elfeed-protocol-owncloud-with-fetch
   (concat elfeed-protocol-owncloud-url elfeed-protocol-owncloud-api-feeds) nil
   (elfeed-protocol-owncloud--parse-feeds)
   (elfeed-protocol-owncloud--do-update 'update-force-all)))

(defun elfeed-protocol-owncloud-mark-read (entry)
  "Notify special ownCloud News entry as read."
  (let* ((id (elfeed-meta entry :id))
         (url (concat elfeed-protocol-owncloud-url
                      (format elfeed-protocol-owncloud-api-read id))))
    (elfeed-protocol-owncloud-with-fetch url "{}")))

(defun elfeed-protocol-owncloud-mark-unread (entry)
  "Notify special ownCloud News entry as unread."
  (let* ((id (elfeed-meta entry :id))
         (url (concat elfeed-protocol-owncloud-url
                      (format elfeed-protocol-owncloud-api-unread id))))
    (elfeed-protocol-owncloud-with-fetch url "{}")))

(defun elfeed-protocol-owncloud-mark-star (entry)
  "Notify special ownCloud News entry as starred."
  (let* ((feed-id (elfeed-meta entry :feed-id))
         (guid-hash (elfeed-meta entry :guid-hash))
         (url (concat elfeed-protocol-owncloud-url (format elfeed-protocol-owncloud-api-star
                                                feed-id guid-hash))))
    (elfeed-protocol-owncloud-with-fetch url "{}")))

(defun elfeed-protocol-owncloud-mark-unstar (entry)
  "Notify special ownCloud News entry as unstarred."
  (let* ((feed-id (elfeed-meta entry :feed-id))
         (guid-hash (elfeed-meta entry :guid-hash))
         (url (concat elfeed-protocol-owncloud-url (format elfeed-protocol-owncloud-api-unstar
                                                feed-id guid-hash))))
    (elfeed-protocol-owncloud-with-fetch url "{}")))

(defun elfeed-protocol-owncloud-mark-read-multi (entries)
  "Notify multiple ownCloud news entries to be read."
  (let* ((url (concat elfeed-protocol-owncloud-url elfeed-protocol-owncloud-api-read-multi))
         (ids (cl-loop for entry in entries collect
                       (when (elfeed-protocol-owncloud-is-owncloud-entry entry)
                         (elfeed-meta entry :id))))
         (data (json-encode-list (list (cons 'items ids)))))
    (when ids
      (elfeed-protocol-owncloud-with-fetch url data))))

(defun elfeed-protocol-owncloud-mark-unread-multi (entries)
  "Notify multiple ownCloud news entries to be unread."
  (let* ((url (concat elfeed-protocol-owncloud-url elfeed-protocol-owncloud-api-unread-multi))
         (ids (cl-loop for entry in entries collect
                       (when (elfeed-protocol-owncloud-is-owncloud-entry entry)
                         (elfeed-meta entry :id))))
         (data (json-encode-list (list (cons 'items ids)))))
...
(defun elfeed-protocol-owncloud-mark-star-multi (entries)
  "Notify multiple ownCloud news entries to be starred."
  (let* ((url (concat elfeed-protocol-owncloud-url elfeed-protocol-owncloud-api-star-multi))
...
(defun elfeed-protocol-owncloud-mark-unstar-multi (entries)
  "Notify multiple ownCloud news entries to be unstarred."
  (let* ((url (concat elfeed-protocol-owncloud-url elfeed-protocol-owncloud-api-unstar-multi))
         (items (cl-loop for entry in entries collect
                         (when (elfeed-protocol-owncloud-is-owncloud-entry entry)
                           (let* ((feed-id (elfeed-meta entry :feed-id))
                                  (guid-hash (elfeed-meta entry :guid-hash)))
...
(defun elfeed-protocol-owncloud-sync-tag-multi (entries tag action)
...
`elfeed-protocol-owncloud-star-tag', ACTION could be add or remove."
  (cond
   ((eq action 'add)
    (cond
     ((eq tag 'unread) (elfeed-protocol-owncloud-mark-unread-multi entries))
     ((eq tag elfeed-protocol-owncloud-star-tag) (elfeed-protocol-owncloud-mark-star-multi entries))))
   ((eq action 'remove)
    (cond
     ((eq tag 'unread) (elfeed-protocol-owncloud-mark-read-multi entries))
     ((eq tag elfeed-protocol-owncloud-star-tag) (elfeed-protocol-owncloud-mark-unstar-multi entries))))))

(defun elfeed-protocol-owncloud-pre-tag (entries &rest tags)
  ...
  (dolist (tag tags)
    (let* ((entries-modified (cl-loop for entry in entries
                                      ...
                                      collect entry)))
      (elfeed-protocol-owncloud-sync-tag-multi entries-modified tag 'add))))

(defun elfeed-protocol-owncloud-pre-untag (entries &rest tags)
  "Sync unread/starred stats before tags removed."
  (dolist (tag tags)
    (let* ((entries-modified (cl-loop for entry in entries
                                      when (elfeed-tagged-p tag entry)
                                      collect entry)))
      (elfeed-protocol-owncloud-sync-tag-multi entries-modified tag 'remove))))
